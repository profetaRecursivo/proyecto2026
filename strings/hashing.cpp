...
struct Hash {
	...
	ll modify(int idx, char nuevo) const {
		int n = sz(h) - 2;
		bint Hraw = h[n];
		ll origVal = get(idx, idx);
		bint pow = pows[idx - 1];
		bint nuevoRaw =
			Hraw - (bint)origVal * pow % MOD + (bint)(nuevo)*pow % MOD;
		nuevoRaw = (nuevoRaw % MOD + MOD) % MOD;
		return (ll)nuevoRaw;
	}
};