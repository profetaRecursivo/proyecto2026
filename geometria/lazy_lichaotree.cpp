...
struct Node {
  ...
  void apply(data_t l, data_t r, Line v) {
    line.add(v);
    lazy.add(v);
  }
};
...
void InsertLineKnowingly(Node* &n, data_t tl, data_t tr, Line x) {
...
void InsertLine(Node* &n, data_t tl, data_t tr, data_t l, data_t r, Line x) {
...
void AddLine(Node* &n, data_t tl, data_t tr, data_t l, data_t r, Line x) {
  if (tr < l || r < tl || tl > tr || l > r) return;
  if (n == nullptr) n = new Node();
...
void InsertLine(data_t l, data_t r, Line x) {
  return InsertLine(root, 0, sz - 1, l, r, x);
}

void AddLine(data_t l, data_t r, Line x) {
